...
### 6.2. Колбэки жизненного цикла соединения

Высокоуровневый API предоставляет колбэки для отслеживания жизненного цикла WebSocket-соединений — когда клиент подключается и когда отключается. Это полезно для логирования, отслеживания соединений и управления ресурсами.

Оба колбэка принимают `WsConnectionHdl` (дескриптор соединения) и являются опциональными — сервер работает без них.

#### `set_on_open_callback`

Срабатывает сразу после WebSocket-рукопожатия (TCP-соединение установлено), до начала криптографического рукопожатия ObscuraProto.

```cpp
server.set_on_open_callback([](WsConnectionHdl hdl) {
    std::cout << "[SERVER] Новое WebSocket-соединение" << std::endl;
});
```

#### `set_on_close_callback`

Срабатывает при закрытии WebSocket-соединения, пока дескриптор ещё валиден. Внутренняя очистка (удаление сессии, отмена потоков, обработка ожидающих запросов) происходит после возврата из этого колбэка.

```cpp
server.set_on_close_callback([](WsConnectionHdl hdl) {
    std::cout << "[SERVER] Соединение закрыто" << std::endl;
});
```
